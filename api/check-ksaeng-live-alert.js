// 크생이 방송 시작 상태를 확인하고 디스코드 시작 알림을 보냅니다.
const CHANNEL_ID = "7dce0fc0bde83e33ff93c1c55edfaefe";
...
const ALERT_MESSAGE = `방송 시작!\n${STREAM_URL}`;
const STATE_ID = "chzzk_ksaeng";
...
module.exports = async function handler(request, response) {
  ...
  const discordWebhookUrl = process.env.KSAENG_DISCORD_WEBHOOK_URL;

  if (!supabaseUrl || !serviceKey || !discordWebhookUrl) {
    response.status(500).json({ error: "Ksaeng live alert API is not configured" });
    return;
  }
  ...
  try {
    ...
    console.error("Unable to process Ksaeng live alert", error);
    response.setHeader("Cache-Control", "no-store");
    response.status(502).json({ error: "Unable to process Ksaeng live alert" });
  }
};
...
async function fetchLiveStatus() {
  const response = await fetch(CHZZK_CHANNEL_URL, {
    headers: {
      ...
      "User-Agent": "ksaeng-live-alert/1.0"
    }
  });
